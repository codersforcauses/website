...
const getWeeksForMonthsInYear = (weeks: Array<ContributionWeeks>) => {
  ...
  weeks
    .map((week) => {
      ...
      return months[new Date(firstDay!).getMonth()]
    })
    .forEach((month) => {
...
const GithubHeatmap = ({ data, theme, width, year }: GithubHeatmapProps) => {
  ...
  const HEATMAP_HEIGHT =
    (DAYS_IN_WEEK + 2) * (RECT_DIMENSIONS + RECT_GAP) - RECT_GAP
  const HEATMAP_WIDTH =
    (WEEKS_IN_YEAR + 2) * (RECT_DIMENSIONS + RECT_GAP) - RECT_GAP
  ...
  return (
    <>
          ...
          >
            ...
            {Object.entries(weeksInMonth).map(([month, weeks]) => (
              <div
                key={month}
                className="self-end"
                style={{ gridColumn: `span ${weeks} / span ${weeks}` }}
              >
                {month}
              </div>
            ))}
                ...
                heatmap.map((heatmapBins) =>
                  heatmapBins.map(({ bin, ...val }) => (
                    <Tooltip key={`github-contrib-${val.row}-${val.column}`}>
                      ...
                      <TooltipContent>
                        <p>
                          {contributionText(bin.contributionCount)} on{" "}
                          {format(new Date(bin.date), "MMM do")}
                        </p>
                      </TooltipContent>
                    </Tooltip>
                  )),
      ...
      <div className="mt-4 flex items-center justify-between text-sm">
        ...
        <div
          className="flex select-none font-mono text-xs"
          style={{ gap: RECT_GAP }}
        >
          <span>Less</span>
